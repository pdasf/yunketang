package com.yunketang.content.service.impl;

import com.alibaba.fastjson.JSON;
import com.yunketang.base.exception.YunketangException;
import com.yunketang.content.config.MultipartSupportConfig;
import com.yunketang.content.feignclient.MediaServiceClient;
import com.yunketang.content.feignclient.SearchServiceClient;
import com.yunketang.content.mapper.CourseBaseMapper;
import com.yunketang.content.mapper.CourseMarketMapper;
import com.yunketang.content.mapper.CoursePublishMapper;
import com.yunketang.content.mapper.CoursePublishPreMapper;
...
import com.yunketang.content.model.po.CourseMarket;
import com.yunketang.content.model.po.CoursePublish;
import com.yunketang.content.model.po.CoursePublishPre;
...
import com.yunketang.content.service.TeachplanService;
...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if ("202003".equals(auditStatus)) {
            YunketangException.cast("该课程现在属于待审核状态，审核完成后可再次提交");
        }
        // 1.2 本机构只允许提交本机构的课程
        if (!companyId.equals(courseBaseInfo.getCompanyId())) {
            YunketangException.cast("本机构只允许提交本机构的课程");
        }
        // 1.3 没有上传图片，不允许提交
        if (StringUtils.isEmpty(courseBaseInfo.getPic())) {
            YunketangException.cast("没有上传课程封面，不允许提交审核");
        }
        // 1.4 没有添加课程计划，不允许提交审核
        if (teachplanTree.isEmpty()) {
            YunketangException.cast("没有添加课程计划，不允许提交审核");
        }
        // 2. 准备封装返回对象
    ...
    public void publishCourse(Long companyId, Long courseId) {
        ...
        if (coursePublishPre == null) {
            YunketangException.cast("请先提交课程审核，审核通过后方可发布");
        }
        // 1.2 课程审核通过后，方可发布
        if (!"202004".equals(coursePublishPre.getStatus())) {
            YunketangException.cast("操作失败，课程审核通过后方可发布");
        }
        // 1.3 本机构只允许发布本机构的课程
        if (!coursePublishPre.getCompanyId().equals(companyId)) {
            YunketangException.cast("操作失败，本机构只允许发布本机构的课程");
        }
        // 2. 向课程发布表插入数据
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        ...
        if (course == null) {
            YunketangException.cast("远程调用媒资服务上传文件失败");
        }
    }
    ...
    public Boolean saveCourseIndex(Long courseId) {
        ...
        if (!result) {
            YunketangException.cast("添加索引失败");
        }
        return true;
    }
    ...
    private void saveCoursePublish(Long courseId) {
        CoursePublishPre coursePublishPre = coursePublishPreMapper.selectById(courseId);
        if (coursePublishPre == null) {
            YunketangException.cast("课程预发布数据为空");
        }
    ...
    private void saveCoursePublishMessage(Long courseId) {
        ...
        if (mqMessage == null) {
            YunketangException.cast("添加消息记录失败");
        }
    }
}
