...
import com.yunketang.content.mapper.*;
...
import com.yunketang.content.model.po.*;
...
public class CourseBaseInfoServiceImpl implements CourseBaseInfoService {
    ...
    public CourseBaseInfoDto createCourseBase(Long companyId, AddCourseDto addCourseDto) {
        ...
        if (baseInsert <= 0 || marketInsert <= 0) {
            YunketangException.cast("新增课程基本信息失败");
        }
        // 3. 返回添加的课程信息
    ...
    private int saveCourseMarket(CourseMarket courseMarket) {
        ...
        if (StringUtils.isBlank(charge))
            YunketangException.cast("请设置收费规则");
        if (charge.equals("201001")) {
            Float price = courseMarket.getPrice();
            if (price == null || price <= 0) {
                YunketangException.cast("课程设置了收费，价格不能为空，且必须大于0");
            }
        }
        // 2.7 插入课程营销信息表
    ...
    public CourseBaseInfoDto updateCourseBase(Long companyId, EditCourseDto editCourseDto) {
        ...
        if (!companyId.equals(courseBase.getCompanyId())) {
            YunketangException.cast("只允许修改本机构的课程");
        }
        // 拷贝对象
    ...
    public void delectCourse(Long companyId, Long courseId) {
        CourseBase courseBase = courseBaseMapper.selectById(courseId);
        if (!companyId.equals(courseBase.getCompanyId()))
            YunketangException.cast("只允许删除本机构的课程");
        // 删除课程教师信息
