package com.yunketang.media.service.impl;
...
import com.yunketang.media.mapper.MediaFilesMapper;
...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, byte[] bytes,
        ...
        try {
            ...
            YunketangException.cast("上传过程中出错" + e.getMessage());
        }
        return null;
    }
    ...
    public MediaFiles addMediaFilesToDB(Long companyId, UploadFileParamsDto uploadFileParamsDto, String objectName, String fileMD5, String bucket) {
        ...
        if (insert <= 0) {
            YunketangException.cast("保存文件信息失败");
        }
        ...
        if ("video/x-msvideo".equals(contentType)) {
            ...
            if (processInsert <= 0) {
                YunketangException.cast("保存avi视频到待处理表失败");
            }
        }
        return mediaFiles;
    }
    ...
    public RestResponse mergeChunks(Long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
        ...
        try {
            ...
            YunketangException.cast("创建合并临时文件出错");
        }
        try {
            // 缓冲区
            ...
            try (RandomAccessFile raf_write = new RandomAccessFile(mergeFile, "rw")) {
                ...
                YunketangException.cast("合并文件过程中出错");
            }
            ...
            try (FileInputStream mergeInputStream = new FileInputStream(mergeFile)) {
                ...
                if (!fileMd5.equals(mergeMd5)) {
                    YunketangException.cast("合并文件校验失败");
                }
                log.debug("合并文件校验通过：{}", mergeFile.getAbsolutePath());
            } catch (Exception e) {
                YunketangException.cast("合并文件校验异常");
            }
    ...
    public void addMediaFilesToMinIO(String filePath, String bucket, String objectName) {
        ...
        try {
            ...
            YunketangException.cast("上传到文件系统出错:" + e.getMessage());
        }
    }
    ...
    private File[] checkChunkStatus(String fileMd5, int chunkTotal) {
        ...
        for (int i = 0; i < chunkTotal; i++) {
            ...
            try {
                ...
                YunketangException.cast("创建临时分块文件出错：" + e.getMessage());
            }
    ...
    public File downloadFileFromMinio(File file, String bucket, String objectName) {
        try (FileOutputStream fileOutputStream = new FileOutputStream(file);
            ...
            YunketangException.cast("查询文件分块出错");
        }
        return null;
    }
    ...
    public void addMediaFilesToMinIO(byte[] bytes, String bucket, String objectName) {
        ...
        try {
            ...
            throw new YunketangException("上传到文件系统出错");
        }
    }
...
// 事务失效1：在方法中捕获异常没有抛出（原因：声明式事务底层由AOP实现，会根据返回异常实现事务回滚）
// 事务失效2：非事务方法调用事务方法（原因：调用事务方法的对象不是代理对象，不能开启事务）
// 事务失效3：事务方法调用事务方法并新开事务（原因：同2）
// 事务失效4：@Transaction标记方法不是public
// 事务失效5：抛出的异常与rollbackFor指的异常不匹配，默认rollbackFor为RuntimeException
// 事务失效6：数据库不支持事务
// 事务失效7：事务的传播行为导致事务失效．比如： PROPAGATION_NEVER PROPAGATION_NOT SUPPORTED
